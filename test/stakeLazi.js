...
contract("Staking", (accounts) => {
    ...
    beforeEach(async () => {
        ...
        await erc721.airdrop([user1, user1, user1, user1, user1], ["1 one", "2 two", "3 three", "4 s", "5 e"], { from: owner })
        await erc721.airdrop([user2, user2, user2, user2, user2], ["6 f", "7 f", "8 s", "9 s", "10 s"], { from: owner })
    })
    ...
    //     await staking.stake(ether("20"), lockPeriodInDays, erc721Ids, { from: user1 })
    //     await time.increase(time.duration.hours(1))
    //     await staking.stake(ether("20"), lockPeriodInDays, [6, 7], { from: user2 })
    ...
    it("should stake additional ERC20 tokens and ERC721 tokens", async () => {
        // Staking initial tokens
        await erc20.approve(staking.address, ether("100"), { from: user1 })
        await erc20.approve(staking.address, ether("100"), { from: user2 })
        ...
        await erc721.setApprovalForAll(staking.address, true, { from: user2 })
        const lockPeriodInDays = 30
        await staking.stake(ether("40"), lockPeriodInDays, erc721Ids, { from: user1 })
        await time.increase(time.duration.hours(1))
        await staking.stake(ether("50"), lockPeriodInDays, [6, 7], { from: user2 })
        await time.increase(time.duration.hours(2))

        // Staking additional tokens
        // await erc20.approve(staking.address, ether("50"), { from: user1 })
        // await erc20.approve(staking.address, ether("75"), { from: user2 })
        await time.increase(time.duration.hours(3))
        await staking.flexibleStake(ether("30"), 30, [4, 5], { from: user1 })
        await time.increase(time.duration.hours(4))
        await staking.flexibleStake(ether("20"), 60, [8, 9], { from: user2 })
    ...
    it("should distribute rewards correctly after one day", async () => {
        ...
        const userAERC721TokenIds = [1, 2, 3,4,5]
        const lockPeriodInDays = 30
        ...
        const userBERC721TokenIds = [6, 7,8,9]
